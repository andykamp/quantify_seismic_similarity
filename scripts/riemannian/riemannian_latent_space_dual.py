...
class RiemannianMetric(object):
    def __init__(self, x1, z1, x2, z2, session):
        self.x1 = x1
        self.z1 = z1
        self.x2 = x2
        self.z2 = z2
    ...
    def create_tf_graph(self):
        ...
        J1 = [tf.gradients(self.x1[:, _], self.z1)[0] for _ in range(28)] # TODO HARDCOD RMEOOOOOOOOOVE
        J1 = tf.stack(J1, axis=1)  # batch x output x latent
        self.J1 = J1

        J2 = [tf.gradients(self.x2[:, _], self.z2)[0] for _ in range(28)] # TODO HARDCOD RMEOOOOOOOOOVE
        J2 = tf.stack(J2, axis=1)  # batch x output x latent
        self.J2 = J2

        G1 = tf.transpose(J1, [0, 2, 1]) @ J  # J^T \cdot J
        self.G1 = G1

        G2 = tf.transpose(J2, [0, 2, 1]) @ J  # J^T \cdot J
        self.G2 = G2

        # add togheter sigma and variance
        self.G = G1+G2
